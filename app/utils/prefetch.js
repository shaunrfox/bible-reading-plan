...
async function fetchAllData(startDate, endDate) {
  ...
  const validData = (await Promise.all(dataPromises)).filter(
    (entry) => entry !== null
  );

  // Create a Map from existing readings for quick lookup
  const readingsMap = new Map(
    db.data.readings.map((entry) => [entry.date, entry.data])
  );
...
// node app/utils/prefetch.js 2024-07-01 2024-07-31
// db1.json = 2024-11-01 – 2025-10-31
// db2.json = 2025-11-01 – 2026-10-31
// db3.json = 2026-11-01 – 2027-10-31
